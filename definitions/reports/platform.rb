    ...
    class Platform < ForemanMaintain::Report
      ...
      def run
        ...
        users_per_mail_notification =
          ...
          query(
            <<-SQL
              select max(mail_notifications.name) as notification_name, count(user_mail_notifications.user_id)
              from user_mail_notifications inner join mail_notifications on mail_notification_id = mail_notifications.id
